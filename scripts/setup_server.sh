...
ALLOWED_CHAT_IDS=
OBSERVER_CHAT_IDS=
# Экран поля, обводка контура, снимок, заметки, окна опрыскивания.
# Пусто = открыто ВСЕМ. Закрыть = вписать chat_id через запятую.
FIELD_STATUS_CHAT_IDS=
# Страница обводки поля. Пусто = кнопки карты нет, только обход углов.
MINIAPP_URL=https://suv-ai.online/draw.html
# Куда пересылать вопросы фермеров свободным текстом.
ADMIN_CHAT_IDS=
