...
def enabled() -> bool:
    ...
    Гейт живёт в окружении, а не в коде: пусто = выключено, и бот ведёт
    себя ровно как до появления второго источника — та же рекомендация
    из того же Sentinel-2. С 09.09.2026 шаблоны (.env.example и
    scripts/setup_server.sh) ставят LANDSAT_FALLBACK=1, то есть новый
    сервер поднимается уже с запасным источником; на работающем сервере
    строку правят руками, деплой чужой .env не трогает.
    """
