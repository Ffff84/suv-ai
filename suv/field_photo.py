...
# Порог площади. ТЗ ставило 3 га, выводя его из подложки 10 м/пиксель —
# но влажность меряется каналом B11 с нативными 20 м, и порог выведен не
# для той величины, которой мы пользуемся. Проверено на настоящем
# винограднике 1,5 га: размах NDMI 0,21 при пороге ровности 0,08, и он
# не проседает при отсечении приграничной кромки в 20 м — то есть
# перепад настоящий, внутри поля, а не смесь с соседями. Ниже 1,2 га
# внутренних клеток замера остаётся меньше десятка — там уже нечего
# показывать.
MIN_AREA_HA = 1.2
...
def can_show_photo(area_ha: float | None, irrigation_method: str,
    ...
    # Способ полива НЕ проверяется, и это осознанное отступление от ТЗ
    # §4.6. «Только борозда» относилось к заливке вдоль борозды; мы же
    # показываем ИЗМЕРЕННУЮ влажность, а замеру всё равно, как поливают.
    # Для капли карта даже нужнее: забитый капельник не видно никак,
    # а сухое пятно вокруг него на снимке читается.
